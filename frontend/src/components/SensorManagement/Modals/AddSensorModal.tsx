...
import { addSensor, AddSensorRequest, addSensorToZone } from '../../../services/sensorService';
import { getAllZones } from '../../../services/zoneService';

interface AddSensorModalProps {
    isOpen: boolean;
    onClose: () => void;
    onAdd: (sensorData: any) => void;
}

const AddSensorModal: React.FC<AddSensorModalProps> = ({ onClose, onAdd }) => {
    const initialFormState: AddSensorRequest & { zoneId?: string } = {
        name: '',
        mqttTopic: '',
        description: '',
        mqttHost: '',
    ...
    const [formData, setFormData] = useState<AddSensorRequest & { zoneId?: number }>(initialFormState);
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        if (type === 'number') {
            ...
            // Handle zone selection - convert to number or undefined
            const zoneValue = value === '' ? undefined : value;
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            const payload = {
                ...
                dataType: 'number', // Keep as number if that's what you want
                unit: formData.unit || '',
            ...
            console.log('🚀 Step 1: Creating sensor with payload:', payload);
            const newSensor = await addSensor(payload as any);
            console.log('✅ Step 1 Complete: Sensor created:', newSensor);

            // Step 2: Zone assignment - DEBUG THIS PART
            console.log('🔍 Step 2: Checking zone assignment...');
            console.log('formData.zoneId:', formData.zoneId);
            console.log('typeof formData.zoneId:', typeof formData.zoneId);
            console.log('Boolean check (formData.zoneId):', !!formData.zoneId);

            if (formData.zoneId) {
                console.log('✓ Zone selected, proceeding with assignment...');
                console.log('newSensor.id:', newSensor.id);
                console.log('typeof newSensor.id:', typeof newSensor.id);

                try {
                    console.log('🔄 Calling addSensorToZone...');
                    console.log('Parameters:', {
                        zoneId: formData.zoneId,
                        sensorId: newSensor.id
                    });

                    const zoneAssignmentResult = await addSensorToZone(formData.zoneId, newSensor.id);

                    console.log('📨 Zone assignment result:', zoneAssignmentResult);

                    if (zoneAssignmentResult.success) {
                        console.log('✅ SUCCESS: Sensor assigned to zone');
                    } else {
                        console.warn('⚠️ PARTIAL SUCCESS: Sensor created but zone assignment failed');
                        console.warn('Failure reason:', zoneAssignmentResult.message);
                    }
                } catch (zoneError) {
                    console.error('❌ ZONE ASSIGNMENT ERROR:');
                    console.error('Error object:', zoneError);
                    console.error('Error message:', zoneError instanceof Error ? zoneError.message : 'Unknown error');
                    console.error('Error stack:', zoneError instanceof Error ? zoneError.stack : 'No stack trace');
                }
            } else {
                console.log('ℹ️ No zone selected (formData.zoneId is falsy)');
                console.log('Possible reasons:');
                console.log('- User did not select a zone');
                console.log('- zoneId is undefined/null/empty string');
                console.log('- Form state issue');
            }
            ...
            setErrors({
                form: error instanceof Error ? error.message : 'Failed to add sensor'
            });
            setIsLoading(false);
        }
    };
